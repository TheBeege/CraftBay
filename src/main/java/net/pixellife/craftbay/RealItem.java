...
public class RealItem implements Item {
	...
	private static String capitalName(String in) {
		return "" + Character.toUpperCase(in.charAt(0))
				+ in.substring(1, in.length()).toLowerCase();
	}
	...
	public String getDescription() {
		...
		if (canBeDamaged() && stack.getDurability() > 0) {
			sb.append(CraftBayPlugin.getInstance()
					.getMessage("item.damaged.Singular").toString());
			sb.append(" ");
		}
		if (!stack.getEnchantments().isEmpty()) {
			sb.append(CraftBayPlugin.getInstance()
					.getMessage("item.enchanted.Singular").toString());
			sb.append(" ");
		}
		sb.append(getName());
		...
		if (enchantments == null || enchantments.isEmpty()) {
			...
			if (meta instanceof EnchantmentStorageMeta) {
				enchantments = ((EnchantmentStorageMeta) meta)
						.getStoredEnchants();
			}
		...
		if (stack.getType() == Material.MONSTER_EGG) {
			...
			case 63:
				sb.append(" (Ender Dragon)");
				break;
			case 64:
				sb.append(" (Wither)");
				break;
			case 97:
				sb.append(" (Snow Golem)");
				break;
			case 99:
				sb.append(" (Iron Golem)");
				break;
			}
		}
	...
	public String getItemInfo() {
		...
		if (meta instanceof BookMeta) {
			...
			if (book.hasAuthor()) {
				result.append(
						CraftBayPlugin.getInstance().getMessage(
								"item.book.ByAuthor")).append(book.getAuthor());
			}
			int pageCount = book.getPageCount();
			result.append(" (")
					.append(pageCount)
					.append(" ")
					.append(CraftBayPlugin.getInstance().getMessage(
							pageCount == 1 ? "item.page.Singular"
									: "item.page.Plural")).append(")");
		}
		...
		if (meta instanceof LeatherArmorMeta) {
			...
			if (result.length() > 0)
				result.append(" ");
			result.append(color.getRed()).append("r,").append(color.getGreen())
					.append("g,").append(color.getBlue()).append("b");
		}
		if (meta instanceof SkullMeta) {
			SkullMeta skull = (SkullMeta) meta;
			if (skull.hasOwner()) {
		...
		}
		{
			...
			Iterator<Map.Entry<Enchantment, Integer>> iter = enchantments
					.entrySet().iterator();
			while (iter.hasNext()) {
				Map.Entry<Enchantment, Integer> enchantment = iter.next();
				if (result.length() > 0)
					result.append(" ");
				result.append(getEnchantmentName(enchantment.getKey()))
						.append(" ").append(roman(enchantment.getValue()));
			}
		}
		if (meta instanceof EnchantmentStorageMeta) {
			Map<Enchantment, Integer> enchantments = ((EnchantmentStorageMeta) meta)
					.getStoredEnchants();
			Iterator<Map.Entry<Enchantment, Integer>> iter = enchantments
					.entrySet().iterator();
			while (iter.hasNext()) {
		...
		if (meta.hasLore()) {
			...
			if (lore.size() > 0) {
				if (result.length() > 0)
					result.append(" ");
				result.append("\"").append(ChatColor.stripColor(lore.get(0)))
						.append("\"");
			}
			for (int i = 1; i < lore.size(); ++i) {
				result.append(" - ").append("\"")
						.append(ChatColor.stripColor(lore.get(i))).append("\"");
			}
		}
	...
	public String toString() {
		String name = "";
		name += "" + stack.getTypeId() + ":" + stack.getDurability() + " "
				+ stack.getAmount();
		Map<Enchantment, Integer> enchantments = stack.getEnchantments();
