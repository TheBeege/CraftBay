...
public class ItemDelivery implements ConfigurationSerializable {
	...
	public boolean deliver() {
		...
		if (recipient instanceof PlayerMerchant) {
			...
			if (player.getPlayer() != null) {
				if (!player.getPlayer().hasPermission("auction.receive")) {
					if (debugMode && attempt == 1) {
						CraftBayPlugin
								...
								.getLogger()
								.info(String
										.format("DELIVER FAIL item='%s' recipient='%s' location='%s' reason='No Permission'",
												item.toString(),
												recipient.getName(),
												getLocation()));
					}
				...
				if (CraftBayPlugin.getInstance().getBlacklistWorlds()
						.contains(player.getPlayer().getWorld().getName())) {
					if (debugMode && attempt == 1) {
						CraftBayPlugin
								.getInstance()
								.getLogger()
								.info(String
										.format("DELIVER FAIL item='%s' recipient='%s' location='%s' reason='World Blacklisted'",
												item.toString(),
												recipient.getName(),
												getLocation()));
					}
		...
		if (result) {
			String msg = String.format(
					"DELIVER item='%s' recipient='%s' location='%s'",
					item.toString(), recipient.getName(), getLocation());
			if (auction != null) {
				auction.log(msg);
			} else {
			...
			String msg = String
					.format("DELIVER FAIL item='%s' recipient='%s' reason='Player offline'",
							item.toString(), recipient.getName());
			if (auction != null) {
				if (debugMode && attempt == 1)
					auction.log(msg);
			} else {
	 ...
	 *            the item
	 * @param auction
	 *            the associated Auction instance
	 * @return The ItemDelivery instance if it had to be scheduled for the
	 *         future, null otherwise
	 */
	public static ItemDelivery schedule(Merchant recipient, Item item,
			Auction auction) {
		ItemDelivery delivery = new ItemDelivery(recipient, item, auction,
				new Date());
		if (!delivery.deliver()) {
			CraftBayPlugin.getInstance().getAuctionScheduler()
