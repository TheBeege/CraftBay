...
public class CommandParser {
	...
	public void registerCommands(Object instance) {
		for (Method method : instance.getClass().getMethods()) {
			if (method.isAnnotationPresent(SubCommand.class)) {
				...
				CommandAttribute attr = new CommandAttribute(this, subCommand,
						method, instance);
	...
	public void registerParameters(Object instance) {
		for (Method method : instance.getClass().getMethods()) {
			if (method.isAnnotationPresent(Parameter.class)) {
				...
				ParamAttribute attr = new ParamAttribute(param, method,
						instance);
	...
	public void callCommand(CommandSender sender, String cmd, List<String> args) {
		...
		if (attr == null) {
			plugin.warn(sender,
					plugin.getMessage("command.NoEntry").set("cmd", cmd));
			return;
		}
		if (!attr.checkPermission(sender)) {
			plugin.warn(sender,
					plugin.getMessage("command.NoPerm").set("cmd", cmd));
			return;
		}
		try {
			...
			plugin.warn(sender,
					cpe.getLocaleMessage().set(sender).set("cmd", cmd));
			return;
		}
	}

	public List<String> preprocess(List<String> argl)
		...
		if (part != null) {
			throw new CommandParseException(
					plugin.getMessage("command.UnclosedQuote"));
		}
		return result;
	}
