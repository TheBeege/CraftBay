...
public class CommandAttribute {
	...
	public CommandAttribute(CommandParser parent, SubCommand subCommand,
	...
	public void call(CommandSender sender, List<String> argl)
			...
			if (param.takesArgument()) {
				if (!iter.hasNext()) {
					if (++skippedArgs > optionalArgc) {
						throw new CommandParseException(CraftBayPlugin
								.getInstance().getMessage(
										"command.ArgsTooSmall"));
					}
					args[i] = null;
				} else {
		...
		if (iter.hasNext()) {
			throw new CommandParseException(CraftBayPlugin.getInstance()
					.getMessage("command.ArgsTooBig"));
		}
		try {
			...
			for (Throwable thrown = ite; thrown != null; thrown = thrown
					.getCause()) {
				if (thrown instanceof CommandParseException) {
					throw new CommandParseException(
							((CommandParseException) thrown).getLocaleMessage());
				}
			}
			ite.printStackTrace();
			return;
