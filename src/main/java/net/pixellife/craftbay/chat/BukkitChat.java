...
package net.pixellife.craftbay.chat;
...
public class BukkitChat implements ChatPlugin {
	...
	public boolean enable(ConfigurationSection section) {
		...
		if (whitelisted) {
			for (String name : conf.getStringList("whitelist"))
				playerList.add(name);
		} else {
			for (String name : conf.getStringList("blacklist"))
				playerList.add(name);
		}
		return true;
	}
	...
	public boolean isListening(Player player) {
		...
		if (!whitelisted
				&& !playerList.contains(player.getName().toLowerCase()))
			return true;
		else if (whitelisted
				&& playerList.contains(player.getName().toLowerCase()))
			return true;
