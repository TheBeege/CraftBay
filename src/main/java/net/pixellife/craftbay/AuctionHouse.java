...
import net.pixellife.craftbay.event.AuctionCreateEvent;
...
public class AuctionHouse implements Listener {
	...
	public boolean checkCooldown(Merchant merchant) {
		...
		if (date == null)
			return true;
		if (System.currentTimeMillis() - date.getTime() < cooldown * 1000)
			return false;
		return true;
	}
	...
	public Auction createAuction(Merchant owner, Item item,
			MoneyAmount startingBid, boolean takeItems) {
		// check
		if (!checkCooldown(owner)) {
			owner.warn(plugin.getMessage("auction.create.OwnerCooldown")
					.set(owner)
					.set("cooldown", new AuctionTime(getCooldown(owner))));
			return null;
		}
		...
		if (takeItems && !item.has(owner)) {
			owner.warn(plugin.getMessage("auction.create.NotEnoughItems")
					.set(item).set(owner));
			return null;
		}
		double fee = 0.0;
		double tax = 0.0;
		if (!owner.hasPermission("auction.nofee"))
			fee = plugin.getConfig().getDouble("auctionfee");
		if (!owner.hasPermission("auction.notax")
				&& startingBid.getDouble() > plugin.getConfig().getDouble(
						"startingbid")) {
			tax = (plugin.getConfig().getDouble("auctiontax") * (startingBid
					.getDouble() - plugin.getConfig().getDouble("startingbid"))) / 100.0;
		}
		MoneyAmount feetax = new MoneyAmount(fee + tax);
		if (feetax.getDouble() > 0.0) {
			if (!owner.hasAmount(feetax)) {
				owner.warn(plugin.getMessage("auction.create.FeeTooHigh")
						.set(owner).set("fee", feetax));
				return null;
			}
			...
			owner.msg(plugin.getMessage("auction.create.FeeDebited").set(owner)
					.set("fee", feetax));
		}
		// take
		if (takeItems) {
	...
	public void endAuction(Auction auction) {
		...
		if (auction.getWinner() == null) {
			...
			ItemDelivery.schedule(auction.getOwner(), auction.getItem(),
					auction);
			...
			ItemDelivery.schedule(auction.getWinner(), auction.getItem(),
					auction);
		}
	}
	...
	public void cancelAuction(Auction auction) {
		...
		if (auction.getFee().getDouble() > 0.0) {
			...
			auction.getOwner().msg(
					plugin.getMessage("auction.cancel.FeeReturn").set(auction));
		}
	}
}
