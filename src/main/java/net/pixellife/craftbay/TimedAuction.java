...
public class TimedAuction extends AbstractAuction {
	...
	public void tick() {
		...
		getPlugin().getServer().getPluginManager()
				.callEvent(new AuctionTickEvent(this));
		timeLeft -= 1;
	}
	...
	/**
	 * Get the amount that the current winner would have to pay, should he win.
	 */
	@Override
	public MoneyAmount getWinningBid() {
		...
		for (Bid bid : bids) {
			if (!bid.getBidder().equals(winner)) {
				double res = Math.min(bid.getAmount() + minIncrement,
						getMaxBid().getDouble());
	...
	/**
	 * Get the minimum amount that a new bidder has to make to be accepted.
	 */
	@Override
	...
	/**
	 * Insert a new bid and assert descending order of bids by amount. Assume
	 * validity of bid.
	 */
	...
	public boolean bid(Merchant bidder, MoneyAmount bid) {
		if (getState() != AuctionState.RUNNING)
			return false;
		if (bidder.equals(getOwner())
				&& !bidder.equals(BankMerchant.getInstance())) {
			bidder.warn(getPlugin().getMessage("auction.bid.IsOwner").set(this,
					bidder));
			return false;
		}
		if (bidder.equals(getWinner())
				&& bid.getDouble() <= getMaxBid().getDouble()
				&& bidder != BankMerchant.getInstance()) {
			bidder.warn(getPlugin().getMessage("auction.bid.UnderbidSelf").set(
					this, bidder));
			return false;
		}
		if (bid.getDouble() < getMinimalBid().getDouble()) {
			bidder.warn(getPlugin().getMessage("auction.bid.BidTooSmall").set(
					this, bidder));
			return false;
		}
		if (!bidder.hasAmount(bid)) {
			bidder.warn(getPlugin().getMessage("auction.bid.TooPoor").set(this,
					bidder));
			return false;
		}
		...
		AuctionBidEvent event = new AuctionBidEvent(this, bidder, bid,
				oldWinner, oldPrice);
		getPlugin().getServer().getPluginManager().callEvent(event);
		return true;
	}
