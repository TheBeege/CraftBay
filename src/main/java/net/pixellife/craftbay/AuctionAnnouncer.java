...
import net.pixellife.craftbay.event.AuctionBidEvent;
import net.pixellife.craftbay.event.AuctionCancelEvent;
import net.pixellife.craftbay.event.AuctionEndEvent;
import net.pixellife.craftbay.event.AuctionStartEvent;
import net.pixellife.craftbay.event.AuctionTickEvent;
...
public class AuctionAnnouncer implements Listener {
	...
	public void onAuctionTick(AuctionTickEvent event) {
		...
		for (int i : countdown) {
			if (auction.getTimeLeft() == i) {
				Message msg;
				msg = plugin
						.getMessage(auction.getWinner() == null ? "auction.countdown.NoWinner"
								: "auction.countdown.Winner");
		...
		if (reminderTimer <= 0 && broadcastTimer <= 0) {
			Message msg;
			msg = plugin
					.getMessage(auction.getWinner() == null ? "auction.reminder.NoWinner"
							: "auction.reminder.Winner");
			announce(msg.set(auction), true, true);
			reminderTimer = reminderInterval;
		}
		if (broadcastTimer > 0)
	...
	public void onAuctionBid(AuctionBidEvent event) {
		...
		if (oldWinner == null
				|| (newPrice.getDouble() != oldPrice.getDouble() && oldWinner
						.equals(event.getBidder()))) {
			// overbid, first winner or overbid yourself with price change
			// (corner case)
			...
			if (!announced || !newWinner.isListening()) {
				Message pmWinner = plugin.getMessage("auction.bid.Win").set(
						event);
				newWinner.msg(pmWinner);
			}
			if (!announced || !oldWinner.isListening()) {
				Message pmLoser = plugin.getMessage("auction.bid.ToLoser").set(
						event);
	...
	public void onAuctionEnd(AuctionEndEvent event) {
		...
		if (event.hasPaymentError()) {
			auction.getOwner().msg(
					plugin.getMessage("auction.end.OwnerPaymentError").set(
							auction));
			auction.getWinner().msg(
					plugin.getMessage("auction.end.WinnerPaymentError").set(
							auction));
			plugin.broadcast(plugin.getMessage("auction.end.PaymentError").set(
					auction));
		} else if (event.getAuction().getWinner() == null) {
			Message msg = plugin.getMessage("auction.end.NoBid").set(auction);
			Message pm = plugin.getMessage("auction.end.OwnerReturn").set(
					auction);
			...
			if (!auction.getWinner().isListening()) {
				Message pm = plugin.getMessage("auction.end.ToWinner").set(
						auction);
				auction.getWinner().msg(pm);
			}
			if (!auction.getOwner().isListening()) {
				Message pm = plugin.getMessage("auction.end.OwnerSell").set(
						auction);
	...
	public void onAuctionCancel(AuctionCancelEvent event) {
		...
		if (auction.getState() == AuctionState.RUNNING) {
			Message msg = plugin.getMessage("auction.cancel.Announce").set(
					auction, event.getSender());
			announce(msg, true, true);
		} else {
